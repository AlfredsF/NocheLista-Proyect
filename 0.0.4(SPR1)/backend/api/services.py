...
def calcular_precio_total(habitacion_id, fecha_inicio, fecha_salida, jwt_token):
    supabase = get_supabase(use_service_role=False, jwt_token=jwt_token)
    hab = supabase.table("habitaciones").select("precio_diario").eq("id", habitacion_id).execute()
    if not hab.data:
        raise ValueError("Habitación no encontrada")
    precio_diario = hab.data[0]["precio_diario"]
...
def get_metricas_hotel(hotel_id):
    ...
    return {"total_reservas": total_reservas, "ingresos_totales": ingresos, "hotel_id": hotel_id}
