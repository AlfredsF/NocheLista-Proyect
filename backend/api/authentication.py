...
class SupabaseJWTAuthentication(BaseAuthentication):
    def authenticate(self, request):
        ...
        # Si no hay cabecera, lanzamos excepción -> HTTP 401 (en lugar de 403)
        if not auth_header:
        ...
        try:
            ...
            result = supabase.table('perfiles').select('rol, nombre_completo, email').eq('id', user_id).execute()
            if not result.data:
            ...
            user_payload = {
                'user_id': user_id,
                'email': profile.get('email', user_data.get('email')),
