...
import { getHotelDataByApiData, mergeWithApiData } from '../../data/hotelesData';
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const localData = getHotelDataByApiData(data);
        const merged = mergeWithApiData(localData, data);
        setHotel(merged);
        document.title = (merged.nombre || 'Hotel') + ' - NocheLista';
        const habs = data.habitaciones || [];
        setHabitaciones(habs);
        if (habs.length > 0) {
          setHabitacionId(habs[0].id);
        }
        const all = await buscarHoteles().catch(() => []);
      ...
      <main className="container my-5">
        <div className="row mb-4">
          <div className="col-12 text-center text-md-start">
            <h1 className="fw-bold text-uppercase">{hotel.nombre}{hotel.ciudad ? ' (' + hotel.ciudad + ')' : ''}</h1>
        ...
        <div className="row mb-5">
          {(hotel.descripcion_parrafos || []).length > 0 ? (
            hotel.descripcion_parrafos.map((p, i) => (
              <div key={i} className="col-md-4 mb-3">
                <p className="text-muted" style={{ textAlign: 'justify' }}>{p}</p>
              </div>
            ))
          ) : (
        ...
        <div className="row mt-5">
          ...
          <div className="row g-3">
            {otrosHoteles.map((h) => {
              const localH = getHotelDataByApiData(h);
              const imgSrc = localH?.imagen_url || h.imagen_url || '/principal.avif';
              const nombre = h.nombre || localH?.nombre || 'Hotel';
              const estrellas = h.estrellas || localH?.estrellas || 0;
              const ciudad = h.ciudad || localH?.ciudad || 'Quito';
              return (
                  ...
                  <Link to={`/hoteles/${h.id}`} className="text-decoration-none text-dark">
                    <div className="card shadow-sm h-100 text-center border-0 bg-light">
                      ...
                      <div className="card-body p-2">
                        <h6 className="card-title mb-1 fw-bold">{nombre}</h6>
                        <small className="text-warning d-block" dangerouslySetInnerHTML={{ __html: getStarsHtml(estrellas) }}></small>
